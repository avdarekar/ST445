...
Jenner bundle note - the original x cd + libname/filename setup lines
(course network drives) are replaced by the EPAcsv0/EPAcsv1/EPAcsv2/
InputDS/Results/HW7 filerefs and libnames set up in autoexec.sas,
which also builds mock EPA Data.csv, EPA Data (1).csv, EPA Data
(2).csv, InputDS.pm10, InputDS.aqssites, InputDS.methods and an
InputDS AQICAT format - the instructor never distributed these to
students (this repo gitignore excludes them). The three INFILE
targets below read from one flat fileref per file instead of the
original single RawData aggregate fileref plus member-name syntax, a
documented SAS construct Jenner does not yet honor (same regression
test as the sibling t002 bundle, filed upstream, internal tracker
only). The PROC TRANSPOSE VAR range below is shrunk from the original
day1-day140/day145-day247/day250-day273/day141-day144 (247 columns) to
day1-day5 - same mechanics against fewer of the same-shaped columns,
since the original width is an artifact of a specific unavailable
dataset rather than something the assignment logic itself depends on.
The content-level PROC COMPARE at the end compares against a
self-derived reference built from the same merge just above, since no
instructor answer key is available for comparison - the comparison
logic (CompOpts, outdif, outnoequal) is unmodified. The original
descriptor-level PROC COMPARE, which fed on an ODS OUTPUT POSITION=
capture of PROC CONTENTS, is omitted here for the same reason as
t002 - ODS OUTPUT POSITION= does not materialize its target dataset
on Jenner even though PROC CONTENTS itself completes. The final PROC
CONTENTS is kept (unmodified) to show that part still runs. Every
other DATA/PROC step, informat, label, and the data-cleaning logic is
...
*build a reference copy for the compare step below (no instructor
 answer key is available - see bundle note above);
data HW7.hw7dugginsfinal;
  set HW7.HW7DarekarFinal;
run;
...
proc compare base = HW7.hw7dugginsfinal
