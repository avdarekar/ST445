...
Jenner bundle note - the original x cd + filename/libname setup lines
(course network drives) are replaced by the Cities/States/Contract/
Mortgaged/InputDS/Results/HW6 filerefs and libnames set up in
autoexec.sas, which also builds mock Cities.txt, States.txt,
Contract.txt, Mortgaged.txt, InputDS.freeclear and InputDS.renters --
the instructor never distributed these to students (the gitignore
in this repo excludes them). The four INFILE targets below read from
one flat fileref per file instead of the original single RawData
aggregate fileref plus member-name syntax, a documented SAS construct
Jenner does not yet honor (see the note in autoexec.sas). Every
informat, delimiter, firstobs and missover option after each INFILE
target is unmodified. The content-level PROC COMPARE below compares
the assembled dataset against a self-derived reference built from the
same merge just above, since no instructor answer key is available
for comparison - the comparison logic (CompOpts, outdif, outnoequal)
is unmodified. The original second (descriptor-level) PROC COMPARE,
which fed on two ODS OUTPUT POSITION= captures of PROC CONTENTS, is
omitted here -- ODS OUTPUT POSITION= does not materialize its target
dataset on Jenner even though PROC CONTENTS itself completes, a
documented, valid construct Jenner does not yet honor. The final
PROC CONTENTS itself is kept (unmodified) to show that part still
runs - only its ODS OUTPUT capture and the second comparison are
dropped.
;
...
*build a reference copy for the compare steps below (no instructor
 answer key is available -- see bundle note above);
data HW6.hw6dugginsipums2005;
  set HW6.HW6DarekarIpums2005;
run;

*compare content portion;
proc compare base = HW6.hw6dugginsipums2005
